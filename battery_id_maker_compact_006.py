...
from cryptography.hazmat.primitives import hashes,serialization #for keys
from cryptography.hazmat.primitives.asymmetric import ec #for keys
...
def write_block_func(block_index,block_contents):  # FIXED: was write_block_num
    if block_index not in non_writable_block_list:      
        ...
        print(f"block {block_index} is Not Writable")  # FIXED variable name
...
# -------------- YOUR EXACT BLOCK DEFINITIONS (UNCHANGED) --------------
#Sector 0
...
# Signature blocks (empty initially)
block_40 = b""
block_41 = b""
block_42 = b""
block_44 = b""

# FIXED: 4 signature blocks only (64 bytes total)
...
writable_block_list_index = [1,2,4,5,6,8,9,10,12,16,17,18,40,41,42,44]  # REMOVED 45!
...
if uid:
    ...
    print(f"checksum (hex) = {checksum.hex()}")
    ...
    #Sign - FIXED 64 BYTES!
    signature = private_key.sign(checksum, ec.ECDSA(hashes.SHA256()))[:64]  # ✅ EXACTLY 64 bytes
    print(f"signature length: {len(signature)} bytes")

    #verification test - USES 64-BYTE VERSION
    public_key = private_key.public_key()
    try:
        public_key.verify(signature, checksum, ec.ECDSA(hashes.SHA256()))  # Tests 64-byte sig
        print("✅ VERIFICATION PASSED - 64-byte signature is VALID!")
    except:
        print("❌ VERIFICATION FAILED!")
    
    # Split into 4 NFC blocks (EXACTLY 64 bytes)
    block_40 = signature[0:16]
    block_41 = signature[16:32]
    block_42 = signature[32:48]
    block_44 = signature[48:64]

    # Update LAST 4 blocks only
    writable_block_list_contents[-4:] = [block_40, block_41, block_42, block_44]
    print("✅ Signature blocks updated!")

    # Write all blocks
    for block_index,block_contents in zip(writable_block_list_index, writable_block_list_contents):
        print(f"Block {block_index} Writable Contents: {block_contents.hex()}")
