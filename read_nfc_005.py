...
def read_block_func(read_block_num):
    ...
    if pn532.mifare_classic_authenticate_block(uid,read_block_num,nfc.MIFARE_CMD_AUTH_A,key_a):
        ...
        if block_contents:
            usable_block_contents = bytes(block_contents).rstrip(b"\x00") 
            actual_text = usable_block_contents.decode("utf-8","ignore")
            print(f"Contents at block {read_block_num} = {actual_text}")
        else:
