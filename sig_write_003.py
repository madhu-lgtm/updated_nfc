#Updated Code For Using I2c Multiplexer (Use Only Reader1) For Writing the signature (20-01-2026)
#Working
import time 
import busio
import board
...
# 8 EMPTY signature blocks
block_40 = block_41 = block_42 = block_44 = block_45 = block_46 = block_48 = block_49 = b""

# 20 blocks total (12 data + 8 signature)
writable_block_list_contents = [block_1, block_2, block_4, block_5, block_6, block_8, block_9, block_10, block_12, block_16, block_17, block_18, block_40, block_41, block_42, block_44, block_45, block_46, block_48, block_49]
writable_block_list_index = [1,2,4,5,6,8,9,10,12,16,17,18,40,41,42,44,45,46,48,49]
...
if uid:
    ...
    # FULL RSA SIGNATURE (128 bytes - NO truncation!)
    signer = PKCS1_v1_5.new(private_key)
    signature = signer.sign(digest)  # COMPLETE 128-byte signature
    print(f"FULL RSA signature length: {len(signature)} bytes")  # 128!

    # Self-verify FULL signature
    public_key = private_key.publickey()
    verifier = PKCS1_v1_5.new(public_key)
    verifier.verify(digest, signature)
    print("SELF-VERIFICATION PASSED - Full RSA signature VALID!")

    # Split FULL 128 bytes to 8 blocks
    block_40 = signature[0:16]
    block_41 = signature[16:32]
    block_42 = signature[32:48]
    block_44 = signature[48:64]
    block_45 = signature[64:80]
    block_46 = signature[80:96]
    block_48 = signature[96:112]
    block_49 = signature[112:128]

    # Update LAST 8 signature blocks
    writable_block_list_contents[-8:] = [block_40, block_41, block_42, block_44, block_45, block_46, block_48, block_49]
