#
#Turn off rf on one reader when reading the other 
#Working_3
...
buzz_pin = 4
red_led_pin = 14
reader_one_grn_led_pin = 15 #Reader1
reader_two_grn_led_pin = 16 #reader2
ic_switch_pin = 18
...
GPIO.setup(reader_one_grn_led_pin, GPIO.OUT)      # Reader1
GPIO.setup(reader_two_grn_led_pin, GPIO.OUT)     # Reader2
...
def pn532_disable_rf(pn532_inst):
    """Disable RF field on PN532 - prevents interference"""
    try:
        pn532_inst.write_datablock([0xD4, 0x32, 0x00])  # TG_InitAsTarget, no target init
        time.sleep(0.01)
        print("PN532 RF disabled")
    except:
        pass

def pn532_enable_rf(pn532_inst):
    """Re-enable and configure for tag reading"""
    try:
        pn532_inst.SAM_configuration()
        time.sleep(0.05)
        print("PN532 RF enabled")
    except:
        pass


def check_readers():
    if reader1 and reader2 == True:
        GPIO.output(ic_switch_pin, GPIO.HIGH)  # Output high (3.3 V)
        print("Boath tags are verified")
    else:
        print("Tag verification pending")
...
def read_block_func_2(block_index):
    if pn532_2.mifare_classic_authenticate_block(uid2, block_index, nfc.MIFARE_CMD_AUTH_A, key_a):
...
def read_rfid_tags():
    ...
    if uid:
        ...
        try:
            ...
            if verified:
                GPIO.output(reader_one_grn_led_pin, GPIO.HIGH)
...
def read_rfid_tags_2():
    ...
    global uid2
    uid2 = pn532_2.read_passive_target(timeout=0.3)
    ...
    if uid2:
        ...
        data_blocks = []
        for block_idx in data_block_indices:
            block_data = read_block_func_2(block_idx)
            if block_data:
        ...
        try:
            verified = verifier.verify(digest, signature)
            if verified:
                GPIO.output(reader_two_grn_led_pin, GPIO.HIGH)
                switch_state = GPIO.input(ic_switch_pin) # Check the IC switch pin state
                if switch_state == GPIO.LOW:
                    GPIO.output(buzz_pin, GPIO.HIGH)
                    time.sleep(0.4)
...
if __name__ == "__main__":
    while True:
        print("=== Scanning Reader 1 ===")
        pn532_disable_rf(pn532_2)  # Kill reader 2 RF FIRST
        pn532_enable_rf(pn532)
        read_rfid_tags()  # Your existing function (reduce timeout=0.3)
        pn532_disable_rf(pn532)
        
        print("=== Scanning Reader 2 ===")
        pn532_disable_rf(pn532)    # Kill reader 1 RF
        pn532_enable_rf(pn532_2)
        read_rfid_tags_2()
        pn532_disable_rf(pn532_2)
