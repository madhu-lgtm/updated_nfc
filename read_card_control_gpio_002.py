...
GPIO.setup(18, GPIO.OUT)      # Set GPIO18 as output for relay
...
def read_rfid_tags():
    ...
    if uid:
        ...
        for block_idx in data_block_indices:
            ...
            if block_data:
                print(f"  Block {block_idx}: {block_data}") #block_data.hex()[:24]
        ...
        for block_idx in signature_block_indices:
            block_data = read_block_func(block_idx)
            if block_data:
                print(f"  Block {block_idx}: {block_data}") #block_data.hex()
        ...
        try:
            ...
            if verified:
                ...
                print("VERIFICATION PASSED! Connecting RC Controls to the drone")
